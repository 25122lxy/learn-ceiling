...
- B+Tree
  - B+树的内部节点通常不存储关键字的具体信息，内部节点相对B树更小，一次性读入内存的需要查找的关键字也就越多。相比于B树，B+树在内存中需要加载更多的关键字，因为它的内部节点只存储指向子节点的指针。这可以减少内部节点的尺寸，使得一次性读入内存的节点数目更多。因此，在某些情况下，相对于B树，B+树可以减少I/O读写操作的次数，提高查询效率。
...
> ---
>
> 1. 语法解析和预处理：MySQL首先会对接收到的查询语句进行语法解析，检查语句是否符合MySQL的语法规则。然后进行预处理，包括语法分析、语义分析等，以确定查询的执行计划。
> 2. 查询优化器：MySQL会使用查询优化器来生成最优的查询执行计划。优化器会考虑多个因素，包括表的索引情况、统计信息、查询条件等，以确定如何最有效地执行查询操作。
> 3. 执行计划生成：基于查询优化器的输出，MySQL生成一个执行计划，该执行计划定义了查询实际执行时的操作顺序、访问路径等信息。
> 4. 查询执行：MySQL根据生成的执行计划执行查询操作，包括表扫描、索引查找、数据过滤、排序等操作。
> 5. 结果返回：当查询执行完成后，MySQL将查询结果返回给客户端。
